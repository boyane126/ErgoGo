...
func init() {
	config.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			// 「info」业务级别的运行日志，例如：用户登录、用户退出、订单撤销
			// 「warn」需要引起关注的信息。例如：调试时候打印调试信息
			// 「error」记录错误信息。Panic 或 Error。例如：数据库连接错误、HTTP 端口被占用等。一般生产环境使用的等级。

			// 级别上由低到高，level 值设置的级别越高，记录到日志的信息就越少
			// 开发时推荐使用 「debug」或「info」，生产环境下使用「error」
			"level": config.Env("LOG_LEVEL", "debug"),

			// 日志的类型，可选：「single」独立的文件、「daily」按照日期每日一个
